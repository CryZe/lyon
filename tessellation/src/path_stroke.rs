...
use crate::{LineCap, LineJoin, Order, Side, StrokeOptions, TessellationError, TessellationResult};
...
impl<'l> StrokeBuilder<'l> {
    pub fn new(options: &StrokeOptions, builder: &'l mut dyn GeometryBuilder<Vertex>) -> Self {
...
fn get_join_angle(prev_tangent: Vector, next_tangent: Vector) -> f32 {
    let mut join_angle = crate::geom::euclid::Trig::fast_atan2(prev_tangent.y, prev_tangent.x)
        - crate::geom::euclid::Trig::fast_atan2(next_tangent.y, next_tangent.x);
...
#[cfg(test)]
use crate::geometry_builder::{simple_builder, Count, SimpleBuffersBuilder, VertexBuffers};
#[cfg(test)]
